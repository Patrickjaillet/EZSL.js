...
- **Codegen** (`src/codegen/glslGenerator.ts`, `src/codegen/types.ts`): `generateFragmentShader`, `generateFragmentShaderMapped`, `generateVertexShader`, `generateThreeVertexShaderMapped`; types `SourceMap`, `GeneratedFragmentShader`, `Program`, `VertexProgram`, `Uniform`, `Expr`, `EzslType`, `FunctionSignature`, `SourceMappedLine`.
